...
function taobaoStrReact() {
    ...
    try{
        ...
        if(taobaoStr.split(/包裹信息/).length > 1){
            YcodeT = taobaoStr.split(/运单号码/)[1].match(/[1-9][0-9]*/g)[0];
            YcopT = taobaoStr.split(/物流公司|运单号/)[1].replace(/：|:+/g, "").trim();
        }else{
            taobaoStr.split(/物流跟踪/).length > 1 ? YcodeT = taobaoStr.split(/运单号码/)[1].match(/[1-9][0-9]*/g)[0] : "";
            taobaoStr.split(/物流公司/).length > 1 ? YcopT = taobaoStr.split(/物流公司|运单号/)[1].replace(/：|:+/g, "").trim() : "";
        }
    ...
    if(OcodeT == ""){
       ...
       OcodeT = GetQueryString("biz_order_id");
    }
...
function taobaoStrOldRx() {
    ...
    if(YcodeT.length < 6 || YcodeT.length >30 || isNaN(YcodeT-0)){
        YcodeT = "";
    }
